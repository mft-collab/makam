...
export const WarningModal = ({ task, assignee, onClose }: WarningModalProps) => {
  return (
    <div className="fixed inset-0 bg-executive-blue/60 backdrop-blur-md flex items-center justify-center p-8 z-[200]">
      <motion.div 
        initial={{ opacity: 0, scale: 0.9, y: 30 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        className="w-full max-w-2xl bg-surface-elevated p-6 md:p-10 rounded-2xl md:rounded-[24px] relative border-[8px] md:border-[10px] border-red-500/10 shadow-[0_40px_90px_-24px_rgba(239,68,68,0.14)] overflow-y-auto overflow-x-hidden max-h-[90vh] custom-scrollbar"
      >
        ...
        <button onClick={onClose} className="absolute top-8 right-8 text-text-muted/40 hover:text-red-500 transition-colors">
        ...
        <div className="flex flex-col items-center text-center gap-4 md:gap-6 relative z-10">
          <div className="w-16 h-16 md:w-20 md:h-20 bg-red-500/10 rounded-2xl flex items-center justify-center text-red-500 shadow-inner shrink-0">
            <AlertTriangle className="w-8 h-8 md:w-10 md:h-10 stroke-[1.2]" />
          </div>
          
          <div className="flex flex-col gap-2 md:gap-3">
            <h2 className="text-2xl md:text-3xl font-light text-text-heading tracking-[0.3em] font-serif uppercase">İkaz ve Uyarı Belgesi</h2>
            <div className="flex items-center justify-center gap-3 md:gap-4">
              <div className="h-[1px] w-12 bg-red-500/30" />
              <span className="text-[10px] text-red-500 font-medium tracking-[0.5em] uppercase">Makam Disiplin Bildirimi</span>
              <div className="h-[1px] w-12 bg-red-500/30" />
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between w-full px-4 md:px-12 pt-4 gap-8 md:gap-0">
             ...
             <div className="flex flex-col items-center gap-3 shrink-0">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border border-red-500/20 flex items-center justify-center relative">
                   <ShieldAlert className="w-8 h-8 md:w-10 md:h-10 text-red-500/30 stroke-[1]" />
                ...
                <span className="text-[9px] text-red-500 font-medium uppercase tracking-[0.4em]">RESMİ MÜHÜR</span>
             </div>
          ...
          <button 
            onClick={onClose} 
            className="mt-4 md:mt-6 px-10 md:px-12 h-12 md:h-14 tracking-[0.4em] bg-red-500/10 text-red-500 border border-red-500/20 rounded-full text-[10px] md:text-[11px] font-medium uppercase hover:bg-red-500/100 hover:text-white transition-all duration-300 w-full md:w-auto"
          >
            SİSTEME DÖN
          </button>
        </div>
